...
import numpy as np
import torch
from nnunet.training.loss_functions.Hybrid_loss import hybrid_loss
from nnunet.utilities.to_torch import maybe_to_torch, to_cuda
from nnunet.training.data_augmentation.default_data_augmentation import get_moreDA_augmentation
from nnunet.training.loss_functions.deep_supervision import HybridLoss
from nnunet.network_architecture.Calib_mask_label import DeepUNet
...
class CalibflipUNetTrainerV5MaskLabel(nnUNetTrainer):
    """
    ...
    def initialize(self, training=True, force_load_plans=False):
        ...
        if not self.was_initialized:
            ...
            self.loss = HybridLoss(self.loss, self.ds_loss_weights)
            # now wrap the loss
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False ):
        ...
        if self.epoch < 100:
            ...
            self.weight *= (0.9 **(self.epoch-100))
        if torch.cuda.is_available():
            data = to_cuda(data)
            target = to_cuda(target)
        forward_data = [data, target]

        self.optimizer.zero_grad()

        model = self.network(forward_data)
        loss = self.loss(model, target, self.weight)

        if run_online_evaluation:
            self.run_online_evaluation(model["seg_out"], target)

        if '__BraTS2021_00008' in keys:
            self.show_diff_pixel(model["logits"], target, data, self.epoch)
    ...
    def show_diff_pixel(self, mask_label, target, image, epoch):
        target = target[0]
        mask_label = torch.sigmoid(mask_label)
        return super().show_diff_pixel(mask_label, target, image, epoch)
