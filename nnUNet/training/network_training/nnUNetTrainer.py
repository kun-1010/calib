...
from nnunet.evaluation.metrics import ece_binary
...
class nnUNetTrainer(NetworkTrainer):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        self.online_eval_fp = []
        self.online_eval_fn = []
        self.online_eval_ece = []
        self.online_eval_mean_confidence = []
        self.online_eval_pos_frac = []
    ...
    def show_diff_pixel(self, flip_matrix, target, image, epoch):
        ...
        self._save_nii(os.path.join(self.subject_dir, 'target.nii.gz'), target.cpu().detach().numpy(), None, None)
        self._save_nii(os.path.join(self.subject_dir, 'mask_label.nii.gz'), flip_matrix.cpu().detach().numpy(), None, None)
        self._save_nii(os.path.join(self.subject_dir, 'image.nii.gz'), image.cpu().detach().numpy(), None, None)
        #for u in range(0,4):
            ...
            #self._save_nii(os.path.join(self.subject_dir, 'attention'+str(u)+'.nii.gz'), att[0][0][u].cpu().detach().numpy(), None, None)
            ...
            if(flip_matrix.shape == label.shape):
                ...
                if(len(x)>0):
                    ...
                    for i, classname in enumerate(classnames):
                        ...
                        mask_path = os.path.join(self.subject_dir, '{}_mask.png'.format(classname))
                        ...
                        self._save_heatmap(pred_data, mask_plot, mask_path)
    ...
    def _save_nii(self, img_path, data, affine, header):
        '''
    ...
    '''
    计算label1,label2tp,fp,fn,tn
    '''
    def label1_label2(self,label,label2):
        with torch.no_grad():
            num_classes = label2.shape[1]
            tp_hard = torch.zeros(label2.shape[0],num_classes - 1).to(label.device.index)
            fp_hard = torch.zeros(label2.shape[0],num_classes - 1).to(label.device.index)
            fn_hard = torch.zeros(label2.shape[0],num_classes - 1).to(label.device.index)
            axes = tuple(range(1, len(label2.shape)-1))
            for c in range(0, num_classes-1):
                tp_hard[:,c] = sum_tensor((label[:, c] == 1) * (label2[:, c] == 1), axes=axes)
                fp_hard[:,c] = sum_tensor((label[:, c] == 1) * (label2[:, c] != 1), axes=axes)
                fn_hard[:,c] = sum_tensor((label[:, c] != 1) * (label2[:, c] == 1), axes=axes)
    ...
    def run_online_evaluation(self, output, target):
        with torch.no_grad():
            ...
            ece_hard = torch.zeros((target.shape[0], num_classes - 1)).to(output_seg.device.index)
            mean_confidence_hard = torch.zeros((target.shape[0], num_classes - 1)).to(output_seg.device.index)
            pos_frac_hard = torch.zeros((target.shape[0], num_classes - 1)).to(output_seg.device.index)
            probabilities = torch.zeros_like(output_seg,dtype=torch.float).unsqueeze(-1)
            if len(output.shape) == 4:
                probabilities = probabilities.repeat(1, 1, 1, 2)
            else:
                probabilities = probabilities.repeat(1, 1, 1, 1, 2)
            probabilities[..., 0] = output[:, 0]
            #probabilities[..., 0] = output_softmax[:, 0]
            for c in range(1, num_classes):
                probabilities[..., 1] = output_softmax[:, c]
                ece_target = (target == c)
                ece,mean_confidence,pos_frac = ece_binary(probabilities, ece_target, n_bins=10, threshold_range=None, mask=None,
                                       out_bins=None, bin_weighting='proportion')
                ece_hard[:, c - 1] = ece
                mean_confidence_hard[:, c - 1] = mean_confidence
                pos_frac_hard[:, c - 1] = pos_frac
                tp_hard[:, c - 1] = sum_tensor((output_seg == c).float() * (target == c).float(), axes=axes)
                fp_hard[:, c - 1] = sum_tensor((output_seg == c).float() * (target != c).float(), axes=axes)
                fn_hard[:, c - 1] = sum_tensor((output_seg != c).float() * (target == c).float(), axes=axes)

            ece_hard = ece_hard.sum(0, keepdim=False).detach().cpu().numpy()
            mean_confidence_hard = mean_confidence_hard.sum(0, keepdim=False).detach().cpu().numpy()
            pos_frac_hard = pos_frac_hard.sum(0, keepdim=False).detach().cpu().numpy()
            tp_hard = tp_hard.sum(0, keepdim=False).detach().cpu().numpy()
            fp_hard = fp_hard.sum(0, keepdim=False).detach().cpu().numpy()
            fn_hard = fn_hard.sum(0, keepdim=False).detach().cpu().numpy()

            self.online_eval_foreground_dc.append(list((2 * tp_hard) / (2 * tp_hard + fp_hard + fn_hard + 1e-8)))
            self.online_eval_ece.append(list(ece_hard))
            self.online_eval_mean_confidence.append(list(mean_confidence_hard))
            self.online_eval_pos_frac.append(list(pos_frac_hard))
            self.online_eval_tp.append(list(tp_hard))
            self.online_eval_fp.append(list(fp_hard))
            self.online_eval_fn.append(list(fn_hard))

    def finish_online_evaluation(self):
        self.online_eval_ece = np.sum(self.online_eval_ece, 0)
        self.online_eval_mean_confidence = np.sum(self.online_eval_mean_confidence, 0)
        self.online_eval_pos_frac = np.sum(self.online_eval_pos_frac, 0)
        ...
        global_dc_per_class = [i for i in [2 * i / (2 * i + j + k) for i, j, k in
                               ...
                               if not np.isnan(i)]
        global_ece_per_class = [i for i in self.online_eval_ece]
        global_mean_confidence_per_class = [i for i in self.online_eval_mean_confidence]
        global_pos_frac_per_class = [i for i in self.online_eval_pos_frac]
        self.all_val_eval_metrics.append(np.mean(global_dc_per_class))
        self.all_val_ece.append(np.mean(global_ece_per_class))

        self.print_to_log_file('Average global ece:', str(global_ece_per_class))
        self.print_to_log_file('Average global mean confidence:', str(global_mean_confidence_per_class))
        self.print_to_log_file('Average global positive fraction:', str(global_pos_frac_per_class))

        self.print_to_log_file("Average global foreground Dice:", str(global_dc_per_class))
        self.print_to_log_file('Flip sample number is', self.flip_num)
