...
import numpy as np
import torch
from nnunet.training.loss_functions.deep_supervision import KickFlipLoss, CalibflipLoss
from nnunet.utilities.to_torch import maybe_to_torch, to_cuda
from nnunet.training.data_augmentation.default_data_augmentation import get_moreDA_augmentation
from nnunet.network_architecture.Caliblabel_UNet import Caliblabel_UNet
...
class CaliblabelUNetTrainerV1(nnUNetTrainer):
    """
    ...
    def initialize_network(self):
        ...
        self.network = Caliblabel_UNet(self.num_input_channels, self.base_num_features, self.num_classes,
    ...
    def to_one(self, y):
        ...
        result = y.new_zeros((s[0], 1, s[2], s[3], s[4])).to(y.device.index)
        result[bg] = 0
        result[ed] = 1
        result[nt] = 2
        result[et] = 3
    ...
    def show_diff_pixel(self, flip_matrix, target, image, epoch):
        ...
        label2 = softmax_helper(flip_matrix)
    ...
    def _save_heatmap(self, np_img: np.ndarray, diff: np.ndarray, out_path: str):
        if os.path.exists(out_path):
            pass
        else:
            norm_diff = colors.Normalize(-1, 1)
            plt.imshow(np_img, 'gray', interpolation='none')
            # diff = np.ma.masked_where((diff>0)&(diff<0.1), diff)
            # ax_img = sns.heatmap(diff, cmap='seismic')
            ax_img = plt.imshow(diff, cmap='bwr', interpolation='none', norm=norm_diff, alpha=0.5)
            plt.colorbar(ax_img)
            plt.axis('off')
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        calib_out, output, calib = self.network(data)

        loss = self.loss(output, target, calib_out, calib)
        self.show_diff_pixel(calib_out[0], target[0], data, self.epoch)
