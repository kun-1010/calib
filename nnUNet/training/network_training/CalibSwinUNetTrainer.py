...
import numpy as np
import torch
from nnunet.training.loss_functions.deep_supervision import CalibLoss
from nnunet.utilities.to_torch import maybe_to_torch, to_cuda
from nnunet.training.data_augmentation.default_data_augmentation import get_moreDA_augmentation
from nnunet.network_architecture.Calib_swin_UNet import Calib_swin_UNet
...
class CalibSwinUNetTrainer(nnUNetTrainer):
    """
    ...
    def initialize(self, training=True, force_load_plans=False):
        ...
        if not self.was_initialized:
            ...
            # now wrap the loss
            self.loss = CalibLoss(self.loss, self.ds_loss_weights)
            ################# END ###################
    ...
    def initialize_network(self):
        ...
        self.network = Calib_swin_UNet(self.num_input_channels, self.base_num_features, self.num_classes,
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        output, calib = self.network(data)

        loss = self.loss(output, target, calib)
