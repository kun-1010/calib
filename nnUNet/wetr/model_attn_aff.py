...
class WeTr(nn.Module):
    def __init__(self, backbone, num_classes=None, embedding_dim=256, stride=None, pretrained=None, pooling=None,):
        ...
        if pooling=="gmp":
            self.pooling = F.adaptive_max_poov3d
        elif pooling=="gap":
            self.pooling = F.adaptive_avg_poov3d

        self.dropout = torch.nn.Dropouv3d(0.5)
        ...
        #a:缩放因子 mode=fan_out表示权重矩阵的形状是从输出通道数计算得到的
        #权重初始化
        nn.init.kaiming_normal_(self.attn_proj.weight, a=np.sqrt(5), mode="fan_out")

        self.classifier = nn.Conv3d(in_channels=self.in_channels[3], out_channels=self.num_classes-1, kernel_size=1, bias=False)
    ...
    def forward(self, x, cam_only=False, seg_detach=True,):
        ...
        if cam_only:
            cam_s4 = F.conv3d(_x4, self.classifier.weight).detach()
