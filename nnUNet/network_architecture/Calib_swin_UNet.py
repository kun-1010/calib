...
class SwinBlock3D(nn.Module):  # 不会改变输入空间分辨率
    def __init__(self, dim, heads, head_dim, mlp_dim, shifted, window_size: Union[int, List[int]],
        ...
        super().__init__()
        self.attn_proj = nn.Conv3d(in_channels=16, out_channels= 1, kernel_size= 1,bias=True)
        self.window_attention = WindowAttention3D(dim=dim, heads=heads, head_dim=head_dim,
    ...
    def forward(self, x):
        x, attn_ = self.window_attention(x)
        attn_cat = torch.cat(attn_[-3:], dim=1)
        attn_cat = attn_cat + attn_cat.permute(0, 1, 4, 3, 2)
...
class Calib_swin_UNet(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        """
        super(Calib_swin_UNet, self).__init__()
        ...
        self.heads = (3, 6, 9, 12)
        self.head_dim = 32
        self.final = FinalExpand3D(in_dim=64, out_dim=32,
                                   up_scaling_factor=2)
        ...
        for d in range(num_pool):
            ...
            self.calib_blocks_context.append(StackedConvLayers(input_features, output_features, num_conv_per_stage,
                                                               self.conv_op, self.conv_kwargs, self.norm_op,
                                                               self.norm_op_kwargs, self.dropout_op,
                                                               self.dropout_op_kwargs, self.nonlin, self.nonlin_kwargs,
                                                               first_stride, basic_block=basic_block))
        ...
        self.up_stage12 = StageModuleUpScaling3D(in_dims=64, out_dims=64,
    ...
    def forward(self, x):
        ...
        for u in range(len(self.tu)):
            ...
            # if u == len(self.tu)-2:
            #     up12 = self.up_stage12(x)
            seg_outputs.append(self.final_nonlin(self.seg_outputs[u](x)))

        # att = self.final(up12)  # (B,num_classes, X, Y, Z)
        # att = self.out(att)
        up12 = self.up_stage12(seg_outputs[-1])
        ...
        if self._deep_supervision and self.do_ds:
            ...
            return seg_outputs[-1], calib_outputs[-1]
