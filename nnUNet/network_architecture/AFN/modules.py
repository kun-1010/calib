...
from nnunet.network_architecture.AFN.affinty_fused import make_gt_toaffinity, get_groupfeature
import numpy as np
...
class ChannelAttention(nn.Module):
    def __init__(self, channel, ratio=2):
        ...
        self.fc1 = nn.Conv3d(channel, channel // ratio, kernel_size=1, stride=1)
        self.fc2 = nn.Conv3d(channel // ratio, channel, kernel_size=1, stride=1)
...
class GAU(nn.Module):
    ...
    def forward(self, x, y):
        ...
        if self.use_gau:
            y = F.interpolate(y, x.shape[-3:], mode='trilinear', align_corners=True)

            comx = x * y
            resx = x * (1 - y)  # bs, c, d,h,w

            x_sa = self.sa(resx)  # bs, 1, d,h,w
            x_ca = self.ca(resx)  # bs, c, 1, 1, 1
    ...
    def forward(self, input_feature, gt, weights=None):
        if self.temp_size > 1:
            ...
            for i in range(3):
                ...
                feature_pad = F.pad(input_feature, (size,size,size, size, size, size, 0, 0, 0, 0), "constant")
            ...
            feature_pad = F.pad(input_feature, (size, size, size, size, size, size, 0, 0, 0, 0),
                                "constant")
            feature_pad_group = get_groupfeature(feature_pad, size)
            fuse_feature = input_feature
            for direction in range(0, 8):
...
class BaseDecoder(nn.Module):
    def __init__(self, in_channels, out_channels, f_channels):
        ...
        self.up_conv = nn.ConvTranspose3d(in_channels, f_channels, kernel_size=2, stride=2)

        self.conv1 = nn.Sequential(
            nn.Conv3d(f_channels * 2, out_channels, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm3d(out_channels),
            nn.ReLU(inplace=True)
        )
...
class Super_multaffSegdecoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, f_channels, use_topo=True, up=True, bottom=False,
        ...
        self.decoder_s = nn.Sequential(
            nn.Conv3d(out_channels + f_channels, out_channels, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm3d(out_channels),
            nn.ReLU(inplace=True),
            nn.Conv3d(out_channels, out_channels, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm3d(out_channels),
        ...
        self.inner_s = nn.Sequential(
            nn.Conv3d(out_channels, 1, kernel_size=3, padding=1, bias=False),
            nn.Sigmoid()
        )
        ...
        if self.use_topo:
            self.decoder_t = nn.Sequential(
                nn.Conv3d(out_channels + out_channels, out_channels, kernel_size=3, stride=1, padding=1),
                nn.BatchNorm3d(out_channels),
                nn.ReLU(inplace=True)
            )
            ...
            if self.multsize_layer:
                ...
                self.inner_t = nn.Sequential(
                    nn.Conv3d(out_channels, 8 * self.tmp_size, kernel_size=3, padding=1, bias=False),
                    nn.Sigmoid()
                )

                self.weight_layer = nn.Sequential(
                    nn.Conv3d(out_channels, 3, kernel_size=3, padding=1, bias=False),
                    nn.Sigmoid()
                )
            else:
                self.fuse_sf_with_8 = Self_fusegt(temp_size=self.tmp_size, affinity=self.affinity)
                self.inner_t_8 = nn.Sequential(
                    nn.Conv3d(out_channels, 8, kernel_size=3, padding=1, bias=False),
                    nn.Sigmoid()
                )
