...
def make_gt_toaffinity(gt, size):
    # N 1 H W L
    # Check
    gt_pad = F.pad(gt, (size, size, size, size, size, 0, 0, 0, 0, 0), "constant")
...
def get_groupfeature(feature, size):
    # Check
    feature_pad_group = [feature[:, :, 2*size:, size:-size, size:-size],  # right
                         feature[:, :, :-2*size, size:-size, size:-size],  # left
                         feature[:, :, size:-size, :-2*size, size:-size],  # up
                         feature[:, :, size:-size, 2*size:, size:-size],  # down
                         feature[:, :, 2*size:, :-2*size, size:-size],  # right_up
                         feature[:, :, 2*size:, 2*size:, size:-size],  # right_down
                         feature[:, :, :-2*size, :-2*size, size:-size],  # left_up
                         feature[:, :, :-2*size, 2*size:, size:-size]]  # left_down
