...
class Caliblabel_UNet(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        """
        super(Caliblabel_UNet, self).__init__()
        ...
        for d in range(num_pool):
            ...
            self.conv_blocks_context2.append(StackedConvLayers(input_features, output_features, num_conv_per_stage,
                                                              self.conv_op, self.conv_kwargs, self.norm_op,
                                                              self.norm_op_kwargs, self.dropout_op,
                                                              self.dropout_op_kwargs, self.nonlin, self.nonlin_kwargs,
                                                              first_stride, basic_block=basic_block))
        ...
        self.conv_blocks_context2.append(nn.Sequential(
            StackedConvLayers(input_features, output_features, num_conv_per_stage - 1, self.conv_op, self.conv_kwargs,
                              self.norm_op, self.norm_op_kwargs, self.dropout_op, self.dropout_op_kwargs, self.nonlin,
                              self.nonlin_kwargs, first_stride, basic_block=basic_block),
            StackedConvLayers(output_features, final_num_features, 1, self.conv_op, self.conv_kwargs,
        ...
        self.conv_blocks_context2 = nn.ModuleList(self.conv_blocks_context2)
    ...
    def forward(self, x):
        ...
        # Branch 2
        for d in range(len(self.conv_blocks_context2) - 1):
            k = self.conv_blocks_context2[d](k)
            skips2.append(k)
            if not self.convolutional_pooling:
                k = self.td[d](k)

        k = self.conv_blocks_context2[-1](k)

        for u in range(len(self.kf)):
            k = self.kf[u](k)
            k = torch.cat((k, skips2[-(u + 1)]), dim=1)
        ...
        if self._deep_supervision and self.do_ds:
            ...
            return seg_outputs[-1], kick_flip_outputs[-1], calib_outputs[-1]
